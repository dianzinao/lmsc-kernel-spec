...
5. **Priority between scaffold and control primitives** - Scaffold injection **MUST NOT** occur synchronously with C1 (suspend), C3 (abort), or C4 (preempt). When the runtime has already chosen a scaffold and the event queue concurrently surfaces C1 / C3 / C4, the kernel **MUST** first complete the scaffold open + attribute-header injection and advance the envelope stack, then process the event queue; if the distribution chooses the reverse priority, it **MUST** cancel that scaffold decision and emit no `kind=envelope_open` audit. An intermediate observable state where "scaffold open is injected but the attribute header is incomplete" is **forbidden**.
6. **Determinism of the trigger predicate** - The trigger predicate **MUST** produce a consistent result under the same `session_state` snapshot (deterministic); distributions **SHOULD** expose a minimal identifier as `x-<vendor>-scaffold-policy-id: string` in `runtime info.features` so that training-data consumers can distinguish scaffold behaviors across distributions.
