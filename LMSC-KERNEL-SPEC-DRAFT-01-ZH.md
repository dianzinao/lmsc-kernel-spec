...
> **最小 session_state 字段集**· `session_state` 提供给 Sampler Hook 读的最小字段集包含：`{ kv_head_token_index, last_sampled_token_id, envelope_stack_depth, envelope_top_kind, attr_region_active }`；发行版**可**扩展更多字段，但上述为所有实现**必须**暴露的子集。
...
> `<|lmsc_abort|>` 受 `I-ABORT-SCOPED` 限制不属于 envelope 外合法入口。`<lmsc_ref>` 是元素而非 envelope，本规范不把它纳入 envelope 外入口集合。
