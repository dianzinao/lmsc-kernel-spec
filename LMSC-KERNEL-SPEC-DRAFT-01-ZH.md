<!-- Source: 00-introduction-and-scope.md -->
...
<!-- Source: 02-normative-language.md -->
...
- blob 共享命名空间（显式 capability，详见 §5.7 F2）；KV 默认按 `program:<name>/` 前缀按程序隔离（见 §5.7 F1），**跨程序 KV 共享不由本规范定义**；发行版**可**在不破坏 P-1 / P-2 与 §5.7 F1 隔离前提下扩展，但属于发行版扩展，不构成规范级互操作能力；
...
2. 计算 `allowed_window`：`candidate_context` 中同时满足 `scope`、`I-ROLLBACK-BOUND`、pin/input 硬边界、`ROLLBACK_SEARCH_MAX_BYTES` 的闭区间；等价伪码为 `allowed_window = candidate_context ∩ scope_window ∩ structural_boundaries ∩ lookback(ROLLBACK_SEARCH_MAX_BYTES)`，其中 `ROLLBACK_SEARCH_MAX_BYTES=null` 时最后一项为全集；`effective_bound` **必须**记录采用的最近边界。
...
5. 若 `j` 存在但匹配区间 `[j, j+len(P))` 未完整落在 `allowed_window` 内：KV 不变；emit audit `kind=rollback`，details 含 `outcome=pattern_out_of_bound` 与 `effective_bound`；`token_rollback` syscall 返 `E_ROLLBACK_BOUNDARY_HIT`；模型路径仅 emit audit，不向任何调用方返错。
6. 否则：丢弃含匹配首字节的整个 token，使 KV cache 尾部位于该 token 之前，并丢弃其之后的一切（含 rollback envelope 自身）；emit audit `kind=rollback`（details 含 `outcome=truncated` / `pattern_bytes_len` / `truncated_tokens` / `effective_bound`）。

> **截断粒度说明**· “丢弃含匹配首字节的整个 token，KV 尾部位于该 token 之前”是刻意设计：字节级 `last_index_of` 的命中位置不依赖 tokenizer，因而匹配结果跨 tokenizer 可预测；但“含匹配首字节 token”这一截断边界以及 §10.1.1 的状态恢复依赖 tokenizer 切分。跨实现恢复一致性仅在 §16.1 C-14 的“同一 tokenizer 与同一 kernel build”前提下成立。示例：若 `Wait,` 的 `W` 与前一个字符共同组成一个 token，截断后连带这个 token 的剩余字节都一起丢失；训练样本确立这个行为。
...
- **`ROLLBACK_PATTERN_MAX_BYTES` 上限强制** 按级别分：
 - **L0 / L1**：kernel **必须**用 logit mask 保证 rollback envelope 自 open 起在 `ROLLBACK_PATTERN_MAX_BYTES` 字节内产生 close。**达上限处理（确定优先级）**：当下一候选 token 会使 rollback body 字节数 = `ROLLBACK_PATTERN_MAX_BYTES` 且尚未生成 close 时，kernel **必须**按以下顺序选择路径——(a) **优先**强制发出 `</lmsc_rollback>` close（forced close）；(b) **仅当**强制 close 同时会违反 `I-ROLLBACK-NO-NEST` / `I-ROLLBACK-NO-ATTR` 等其他不变量而无法合法关闭时，**才**降级为强制 `<|lmsc_abort|>` 走 C3 abort 路径。两条路径**禁止**由发行版静默互换。Audit 要求：(a) 路径 emit `kind=rollback`，details **必须**含 `forced_close=true` 与 `effective_bound="pattern_max_bytes"`，随后按正常 rollback pattern 语义处理（搜索、命中 / 未命中、KV 截断、`ROLLBACK_STORM_LIMIT` 计数、region 清理），**禁止**形成 body 字节数超过 `ROLLBACK_PATTERN_MAX_BYTES` 的 rollback envelope；(b) 路径 emit `kind=rollback`（details 含 `forced_close=true`、`forced_close_outcome="abort_fallback"`）与 `kind=protocol_violation`（invariant=`I-ROLLBACK-BOUND`），KV 不变；
...
- 若 `<lmsc_rollback>` open **之前**当前推理位置处于某个**非 rollback** envelope 的 body 内（即 envelope 栈上至少存在一个非 rollback envelope 帧）→ `scope = Envelope`；
- 否则（栈上无任何非 rollback envelope，含 `GENERATING` 与仅有 rollback envelope 自身在栈的两种情形）→ `scope = Turn`。

> **rollback envelope 不参与 scope 推定**· 即便在处理 `</lmsc_rollback>` close token 的瞬间 rollback envelope 仍位于 envelope 栈顶，scope 推定**只**考察「rollback open 之前」的栈状态；rollback envelope 自身**禁止**被计为「非 rollback envelope body」。本节为 §13.3 「模型路径默认 scope 推定」的规范正源，§13.3 等处仅作引用，**禁止**引入与本节冲突的推定文本。

推定结果写入 §14.2 rollback audit details 的 `search_scope` 字段；顶层 `AuditRecord.scope` 仍只表示审计可见性，**禁止**复用来表示 rollback 搜索范围。
...
- 转义后匹配：若程序原始写入 `<lmsc_output>`，按 §13.2 实际进入 context 的字节为 `‹lmsc_output>`，rollback pattern **必须**匹配 `‹lmsc_output>`，而不是未转义原文；
...
其中首个属性前**必须**有空白分隔符，属性头以一个空行结束；空行之后才是 envelope body。属性头只在 kernel 已将该字节区间标记为 `kernel-owned attr_header` 时才被解析为属性。模型或程序在普通 body 中生成相同字面时，只按 body 字节处理，**禁止**被提升为属性。每个属性值的 detokenized UTF-8 字节长度**必须** ≤ `ATTR_VALUE_MAX_BYTES`（默认 256，见附录 F）；超限时 kernel **必须**拒绝该属性头或 syscall 输入并返回 `E_EMIT_ESCAPE_VIOLATION`。
...
> **属性头注入原子性（通用规则）**· 任何非 rollback envelope 的 open token 与紧随其后的 kernel-owned 属性头**必须**作为不可分割写入单元完成：从 open token 写入开始到属性头终止空行写入完成之间，kernel **禁止**穿插处理 C1（suspend）/ C3（abort）/ C4（preempt）等控制事件，事件队列在此期间到达的控制事件**必须**排队等待属性头注入完成后再处理；亦**禁止**让程序态或推理栈出现「open 已写入但属性头未完成」的中间可观测状态。本规则**覆盖所有 kernel 补属性头的路径**——模型自发 open 后由 kernel 注入缺省属性头、`envelope.emit` / boot output / persona-refresh 写入、auto execute scaffold 注入（§10.1.2）、以及实现接受 open-tag 形式属性输入再规范化的路径——而非仅限于 scaffold 子模式。若实现因任何原因决定放弃本次写入，**必须**在 open token 写入**之前**取消并回滚相关决策，**禁止** emit `kind=envelope_open` audit。§10.1.2 第 5 项保留 scaffold 子模式下等价但更紧的事件队列细则（与控制事件优先级判定相关），与本通用规则不冲突；冲突时以本节为协议表面正源。
...
>
> **长度预算** · `gen` 仍是自由字符串，但它受统一属性值上限 `ATTR_VALUE_MAX_BYTES` 约束；推荐程序与发行版词汇表**应**将 `gen` 控制在 64 bytes 以内，以便审计、diff 和训练样本稳定对齐。
...
str_literal = '"' , { str_char } , '"' ;
 ...
 (* 每个 str_literal 解码后的 UTF-8 字节长度 <= ATTR_VALUE_MAX_BYTES；gen 建议 <= 64 bytes。 *)
...
>
> **字符禁用显式声明** · 上述 `ref_char` 字符集已隐含排除双引号 `"` 与分号 `;`——两者是 ref body 分隔符本身。此处显式重申：`kind` / `id` 的值**禁止**含 `"` 与 `;`。若外部系统 ID 含非白名单字符（中文、空格、Unicode 标点、引号、分号等），程序**必须**在 `ref.bind` 阶段自行做映射/哈希处理，kernel **不**提供转义机制；kernel 对违规 ref body 一律返 `E_REF_BODY_INVALID`。非规范互操作建议见 §9.3：优先使用 `base64url(no_padding(utf8_bytes))` 或 `sha256:<hex-prefix>` 一类 canonical id，并登记 scheme / collision 策略。
...
- **I-ATTR-CLEAN** · kernel-owned attribute header 以空行结束为边界；属性头内**禁止**出现任何特殊 token 字面，且 header 前缀、首属性前分隔符、行尾、空行与属性值长度上限**必须**符合 §4.3 / §4.2。
...
- **I-STRICT-ENVELOPE-ONLY**（**条件不变量**，仅当 `features.strict_protocol_mode=true` 且 `features.raw_text_outside_envelope="forbidden"` 时启用）· 在 `GENERATING` 状态且 envelope 栈深 = 0 时，模型采样的下一 token **必须**属于「合法 LMSC 协议入口」集合：`<lmsc_execute>` / `<lmsc_output>` / `<lmsc_edit>` / `<lmsc_rollback>` / `<|lmsc_suspend|>` / `<lmsc_ref>` open / EOS 之一；任一非入口的普通 token 视为违反。`<lmsc_ref>` open 之所以纳入入口集，是为与 `I-REF-ANY`（ref **可**出现在任何 envelope body 与普通 chunk 中）保持一致；ref element 关闭后栈深仍为 0，紧随其后的下一 token 仍受本约束。若该 ref element 是在本不变量激活且栈深 = 0 时开启，成功展开结果还**必须**按 §9.3 strict 顶层 ref materialization 处理，kernel **禁止**把 resolver 返回的普通 bytes 仅经 `I-EMIT-ESCAPE` 后直接写入 envelope 外 context。L0 / L1 **必须**通过 sampler hook logit mask 在采样前强制；L2 由 token stream hook 事后识别后走 `kind=protocol_violation`（invariant=`I-STRICT-ENVELOPE-ONLY`）路径，与 §6.5.1 第 10 条同等处理，返 `E_PROTOCOL_VIOLATION`。当 `raw_text_outside_envelope="audit-only"` 时，本不变量**不**强制，但 kernel **必须** emit audit `kind=raw_text_outside_envelope`（不构成 violation，即使 `strict_protocol_mode=false`）。当 `raw_text_outside_envelope="allowed"` 时，本不变量**不**生效；`strict_protocol_mode=false` 且 `raw_text_outside_envelope="forbidden"` 为非法配置，详见 §10.1.2。
...
<!-- Source: 05-atomic-primitives.md -->
...
核心原语 **24 条**必须实现；**级别相关可选** **A2、H3** 两条按 §6.5 接入级别决定：L0 必须提供 A2 与 H3，L1 可选择提供，L2 可不提供。
...
- **前置**：无。
- **后置**：只读；返回映射在会话周期内不变。
- **备注**：等价信息已在 boot output 返回；本原语是便利 API。合规 L0 实现**必须**提供；L1 实现**可**提供；L2 实现**可**不提供。程序应在 `on_load` 缓存 boot output 的 tokens 清单，并在调用前检查 `runtime info.kernel.atomics`。
...
- **后置**：
 ...
 - 命中：丢弃包含匹配首字节的整个 token，KV cache 尾部位于该 token 之前（含 rollback envelope 自身一并丢弃）；
...
模型触发 `<lmsc_rollback>…</lmsc_rollback>` envelope 与程序调此原语**语义等价**；两条路径共享上述前置与不变量。模型路径未命中不报错，仅 emit audit。**模型路径 `scope` 推定**引用 §4.1.2a 正源：若 `<lmsc_rollback>` open 之前栈上至少存在一个非 rollback envelope 帧，则 `scope=Envelope`；否则 `scope=Turn`；rollback envelope 自身不参与推定。程序路径的 syscall 签名不提供默认值，调用方**必须**显式传递 `scope`；语言绑定或 kernel 收到缺省/缺失 scope 时**必须**拒绝该调用并返回 `E_PROTOCOL_VIOLATION` 或绑定层等价错误。
...
| L1 | `byte` 或 `token` | 命中后通过 rollback truncate adapter 截断；`token` 分支丢弃包含匹配首字节的整个 token，KV 尾部位于该 token 之前 | 命中并完成截断后进入 |
...
- **前置**：
 - 状态 **必须**处于 `{ GENERATING, IN_ENVELOPE, DISPATCHING }`；若会话处于 `SUSPENDED`，普通 A4 syscall **必须**返回 `E_INJECT_BAD_POSITION`，details 建议置 `reason="suspended"`。C2 `resume` payload 的注入是 C2 的原子恢复路径，不视作在 `SUSPENDED` 内直接调用 A4；
 - 所有 tokens 在词表内；
 - 若包含保留 token，持 `tokens.inject.special` cap（fixed-only）；
 - 总长度 ≤ `INJECT_MAX_TOKENS`（默认 4096）；
 - 注入后 envelope 栈深**不得**超过 `MAX_ENVELOPE_NEST`；会导致栈溢出的注入 **必须**返回 `E_STACK_OVERFLOW`；
 ...
 - **Token 序列内容不变量（所有 position 适用）**：当注入位置进入**非 rollback** envelope body（含 `position=AtEnvelopeClose` 默认场景，以及 `position=BeforeNextSample` 时当前栈顶为非 rollback envelope）时，注入 token 序列**必须**满足 §4.4 `I-CTRL-INSIDE-ENV`：仅允许出现 `<|lmsc_abort|>`、`<|lmsc_suspend|>`、完整的 `<lmsc_ref>...</lmsc_ref>` ref element、以及嵌套的 `<lmsc_rollback>` open 等许可保留 token；其他任意 envelope 的 open / close token（含 `<lmsc_execute>` / `<lmsc_output>` / `<lmsc_edit>` 与同类型嵌套 open / close）**禁止**注入，即便调用方持有 `tokens.inject.special` cap。kernel **必须**在 syscall 入口（采样 / 写入 KV 之前）对完整 token 序列做该校验；违反者 **必须**返回 `E_INJECT_BAD_TOKEN`，details 建议含 `reason="ctrl_inside_env_violation"`，并 emit audit `kind=protocol_violation`（invariant=`I-CTRL-INSIDE-ENV`）。envelope 外注入仍按既有「**禁止**注入 envelope close token」等规则处理；rollback envelope body 已由上一条 `AtEnvelopeClose` 禁用规则与 `I-ROLLBACK-NO-NEST` 共同覆盖。
- **Position 语义**：
...
- **错误**：`E_INJECT_BAD_TOKEN` / `E_INJECT_BAD_POSITION` / `E_INJECT_CAP_DENIED` / `E_INJECT_TOO_LARGE` / `E_STACK_OVERFLOW`。

### A5 · `session_fork`

```
session.current_handle() -> SessionHandle # 当前 session 句柄，不创建 fork
...
- **前置**：
 ...
 - `fork`：会话内任一 handler **禁止**持有进行中的流式 `envelope.emit`；违反本条件时**必须**返 `E_EMIT_STREAM_OPEN`，details 至少包含 `reason="streaming_emit_in_progress"`、`context="session.fork"` 与 `open_stream_count`，并应包含可暴露的未关闭 `envelope_id` 列表；持 `session.fork` cap。dispatch 内调用合法——`runtime admin fork`（§12.3）即依赖本路径。（流式 emit 未完结时不可分）
...
- **后置**：
 - `current_handle` 返回当前 session 的不透明 handle；该操作不改变 context / KV / audit cursor / dispatcher frame / fork 配额状态；
...
**Dispatch timeout 与 handler panic 的优先级**· 同一 dispatch 只能产生一个主错误码。若 kernel 已在单调时钟上判定 `constraints.max_dispatch_ms` 到期，则主结果**必须**为 `E_DISPATCH_TIMEOUT`，即使随后观察到 handler panic；audit details **应**包含 `secondary_error="handler_panic_after_timeout"`（若可安全确认）。若 handler panic 在 deadline 到期前已被 kernel 观察并进入 close 路径，则主结果为 `E_HANDLER_PANIC`。若两者在同一调度 tick 内不可区分，timeout 优先，以避免过期 handler 通过 panic 改写超时归因。两条路径仍共享 `on_abort` 去重规则。
...
- **后置**：
 ...
 - **attrs 值字面级约束**：`attrs` map 的任一值若含保留 token 字面序列，或解码后的 UTF-8 字节长度超过 `ATTR_VALUE_MAX_BYTES`，kernel **必须**直接拒绝返 `E_EMIT_ESCAPE_VIOLATION`；**不**做视觉等价改写（保持 attribute header 的清洁性与 `I-ATTR-CLEAN` 对齐）。
...
- **未关闭流式 emit 的判定窗口**：自 open token 注入（或 L2 buffered 模式下完成缓冲并进入 open-token 注入路径）起，直到 close token 注入、异常 abort 完成或 buffered reject-before-open 丢弃缓冲为止，均视为未关闭；包含因背压、暂停等待下游消费、generator 阻塞或 observer 回调尚未返回而停顿的状态。
...
- 当外部 abort、observer reject、rollback 截断、dispatch timeout 或 turn abort 取消一个正阻塞在 `envelope.emit` 上的流式调用时，调用方可观察的 syscall 结果**必须**为 `E_EMIT_STREAM_ABORTED`（超时专用路径可为 `E_EMIT_STREAM_BACKPRESSURE_TIMEOUT`）；audit / callback details 再用 `terminate_reason` 或 `reason` 区分具体触发源。
...
- **后置**：**可选** `token_inject` payload；该 payload 由 C2 在 resume 原子路径内注入，随后推理栈回到 GENERATING；触发 handler `on_event`（若注册）。该 payload 不赋予程序在 `SUSPENDED` 状态直接调用 A4 的权限。**Payload 上限**：`payload` 的 token 总数**必须** ≤ `INJECT_MAX_TOKENS`（默认 4096，与 A4 同源），超出**必须**返回 `E_INJECT_BAD_TOKEN`（details 建议 `reason="resume_payload_overflow"`）并保持原 SUSPENDED 状态、不消耗 handle；此约束防止通过 `suspend` / `resume` 循环绕过 A4 的注入配额。非空 payload **必须**按 §13.3 input region 创建规则登记（来源由发行版声明的 external_input bridge 标识）；`payload=NULL` 或空序列按下述零 token 规则处理。
- **C2 payload 边界细则**· `payload=NULL` 与空序列不会产生非空 input region 区间；kernel **必须**在 `resume` audit / runtime-query 元数据中记录 `payload_tokens=0` 与 `input_region_created=false`（或等价布尔），并**禁止**创建零长度 rollback 硬边界。非空 payload 创建的 input region **必须**精确覆盖其注入 token 区间；若 resume 点处于 `IN_ENVELOPE`，payload 注入仍是 C2 原子恢复的一部分，但该 input region **必须**作为 rollback 硬边界，后续 rollback **禁止**从 envelope 外或 payload 后方跨入该区间匹配。C2 payload **禁止**包含任何会使当前 envelope 栈跨越、提前关闭或伪造新 envelope 的保留 token；违反时返回 `E_INJECT_BAD_TOKEN`（details `reason="resume_payload_ctrl_violation"`），保持 SUSPENDED 状态且不消耗 handle。
- **错误**：`E_SUSPEND_EXPIRED` / `E_SUSPEND_INVALID` / `E_INJECT_BAD_TOKEN`。
...
- **`program_whitelist` 组合语义**：`None` 表示该 frame 不增加白名单约束；`Some(non_empty)` 表示只允许其中列出的 `argv[0]`；`Some([])` 表示显式拒绝所有程序。有效白名单为基础 dispatch 可达程序集合与栈上所有非 `None` `program_whitelist` 的交集。dispatcher frame **只能收紧**可调用程序集，**不得**扩大底层 manifest、capability 或更外层 frame 已经施加的限制。
- **基础 dispatch 可达程序集合（base set）定义**· 在**不考虑**当前 dispatcher frame 栈上任何 `program_whitelist` 限制的前提下，按以下条件过滤后、本会话当下**可被** dispatch 的全部 `argv[0]` 集合即为 base set：
 1. 程序已在当前 session 中按 §8 完成 `program.register` 且 `on_load` 成功（**不**含 `status=loading` 或已 `program.unregister` 的程序）；
 2. 调用方 manifest 与已生效 capability 允许 dispatch 该 `argv[0]`（含 `program.dispatch.<name>` 等具名授权）；
 3. 程序自身 manifest 声明的可见性 / 调用来源约束（如 `caller=kernel` / `caller=program:*` / `dispatchable=false`）允许本次调用源；
 4. §12 规定的必要程序路由（`runtime` / `program` / `capability` / `kv`）按规范固定可达，**禁止**被发行版从 base set 中移除；
 5. 发行版 dispatch 策略（如 sandbox / network 隔离 / 出域过滤）未在本会话当下拒绝该 `argv[0]`。

 base set **禁止**因 dispatcher frame 而**扩大**，仅可被 frame stack 上任一非 `None` `program_whitelist` 进一步收紧；有效可调用集合 = base set ∩ frame stack 上所有非 `None` `program_whitelist`。`program.list` 的模型可见视图**应**是 base set 的子集（叠加可见性过滤），**禁止**包含 base set 外的程序；当模型可见视图与 base set 暂时不一致时（如 `on_load` 期间），**以** base set 作为 dispatch 授权正源，`program.list` 仅为提示，调度成败仍由 base set + frame whitelist 判定。
- **拒绝归因顺序**· B2 dispatch 判定**必须**先计算 base set，再应用 frame whitelist：若 `argv[0]` 不在 base set，返回底层原因对应的错误（如 `E_UNKNOWN_PROGRAM` / `E_CAP_DENIED` / `E_PROG_NOT_FOUND`，details 建议 `layer="base_set"`）；若它在 base set 但被当前 frame whitelist 排除，返回 `E_CAP_DENIED`（details `reason="dispatcher_frame_denied"`, `layer="frame_whitelist"`）。实现**禁止**把 frame 拒绝伪装成程序不存在，以便审计能区分注册/授权问题与临时聚焦约束。
...
> **非规范性说明**：blob 的 GC 时机是实现自由度；kernel 仅保证 refcount 归零后**禁止**再返回该 blob 的内容（`blob.get` 返 `E_BLOB_NOT_FOUND`），但具体回收可延迟至批量 GC。程序**应**避免假设 `decref` 调用后内存立即释放。session 结束或 `drop_fork` 释放某一分支时，kernel 对该 session 持有集合中的 BlobId 执行 session-level `decref`；程序显式 `blob.decref` 只能释放调用方 session 所持的那份引用。**最佳实践警告**：若程序把 blob handle 作为普通 bytes 写入 KV 或自身数据结构，kernel 不会扫描该内容；需要跨 fork / drop_fork 保留该 blob 时，应在写入 KV 前后显式调用 `blob.incref`，并在删除引用时配对 `blob.decref`，否则长会话或分支释放后可能出现逻辑上的悬空引用。
 ...
 **允许集** · handler 仅允许进行以下 **5 类**调用，此列表之外的任何 syscall 与路径均**禁用**：
 ...
 2. **本 session turn-local KV 只读**：`kv.get` / `kv.list`（仅限 resolver 所属程序自己的 `program:<resolver-name>/` 动态空间；**禁止**读取全局 KV、`program:kv/` 必要程序命名空间或其他程序命名空间）；
 ...
 **返回内容约束** · resolver 返回值在本规范中是普通 `ResolvedContent` bytes，而不是 typed protocol item；kernel 在 inline materialization 前**必须**按 §9.3 / §13.2 对返回 bytes 执行 reserved-token literal 扫描与 `I-EMIT-ESCAPE` 处理，确保返回内容不能伪造 envelope/control token。返回内容若包含 `<lmsc_ref>` / `</lmsc_ref>` 字面，按 §9.3 递归 ref 禁止返回 `E_REF_BODY_INVALID`；发行版若允许 resolver 返回结构化 envelope-safe item，必须另行声明 parser、审计与合规测试。
...
- **错误**：`E_REF_KIND_TAKEN` / `E_REF_NOT_FOUND` / `E_REF_EXPIRED` / `E_REF_BODY_INVALID` / `E_PROTOCOL_VIOLATION` / `E_CAP_DENIED`。
...
`tag` 打标，attrs 含 `gen` / `visibility` / `attention` / `owner`；attrs 值**必须**遵守 `ATTR_VALUE_MAX_BYTES` 与保留 token 字面禁用约束，违反返 `E_EMIT_ESCAPE_VIOLATION`；`attention=pin` 需 `region.pin` cap（见 §11.4）；`owner` 属性仅供审计与可视化消费，kernel 本身**不**解析。
...
### H3 · `exec_sandbox`（**级别相关可选**）

```
exec.sandbox(kind, code, stdin?, limits) -> ExecResult
```

- 合规 L0 实现**必须**提供；L1 实现**可**提供；L2 实现**可**不提供；
...
<!-- Source: 06-inference-stack-contract.md -->
...
| **L1** | **Tier-B** | Sampler + Token Stream；无完整 KV Hook；必须提供 rollback truncate adapter | A3 `scope=Envelope` 允许按字节精度降级实现，**禁止**强制退化到 turn 起点；**A5 不可用**（`session.fork/restore/drop_fork` 返 `E_ATOMIC_UNSUPPORTED`）；A2 / H3 为级别相关可选（本级可提供） |
...
10. `I-STRICT-ENVELOPE-ONLY`（**条件性**，仅当 `features.strict_protocol_mode=true` 且 `features.raw_text_outside_envelope="forbidden"` 时纳入；L2 检测点：每 token 入流时检查当前 envelope 栈深 = 0 且非 `<lmsc_execute>` / `<lmsc_output>` / `<lmsc_edit>` / `<lmsc_rollback>` / `<|lmsc_suspend|>` / `<lmsc_ref>` open / EOS 即为 violation；返 `E_PROTOCOL_VIOLATION`）。若该入口是栈深 0 的 `<lmsc_ref>`，成功展开还必须按 §9.3 strict 顶层 ref materialization 处理；把普通 resolved bytes 直接落到 envelope 外 context 也视为同一不变量违规。
...
在本隔离窗口内，`<lmsc_ref>` / `</lmsc_ref>` 字面只作为 rollback pattern 的普通字节参与 `I-ROLLBACK-NO-NEST` 检查；§4.4 的 `I-REF-ANY` 不会在 rollback body 内开启 ref 解析例外。实现若把 rollback body 中的 ref token 提升为 `ref.resolve` 事件，即使随后成功清理 context，也已违反本隔离要求。
...
| 合法 rollback 且 pattern 命中 | `token` | 丢弃包含匹配首字节的整个 token，KV 尾部位于该 token 之前；rollback envelope 自身不保留 | `rollback(outcome=truncated, truncate_precision=token)` | 按 §10.1.1 恢复 |
...
<!-- Source: 07-syscall-interface.md -->
...
| `cap.revoke` | D3 | `capability.revoke`† 或原 grantor |
...
`availability=internal-only-v1` 与 `v1_program_accessible: false` 表示 syscall 名称与参数形状被登记，但 V1 程序侧无合法授权路径；实现**禁止**据此向程序开放 `tokens.rollback.explicit`。所有 V1 中程序侧不可达但保留机器形状的 syscall 条目**必须**带等价的显式不可达标记。

每个 `syscalls[]` 条目**必须**包含 `errors` 集合，且该集合至少覆盖对应 §5 原语 `错误` 小节列出的可观测错误码；发行版扩展错误码可追加但不得删除规范错误。错误码的 `origin` 元数据不在 syscall 条目中重复定义，必须来自 §15.2 的错误码注册表（或同构机器文件 `lmsc-error-code-registry.yaml`）；schema 校验器在生成语言绑定时**必须**交叉验证 `errors[]` 中每个错误码均存在对应 origin。
...
> **`program.list` / `program.info` 返回结构的 `status` 字段（规范性）**· 上述视图与 `program.info` 响应**必须**为每条程序记录暴露 `status: "ok" | "loading" | "unloading"` 字段：`ok` 表示 `on_load` 已成功且未进入 `on_unload`；`loading` 见 §8.2；`unloading` 表示 `on_unload` 调度中（仅 `program.admin` 可见且不可作为 dispatch 目标）。发行版**可**追加自有 status 值，但**禁止**省略本字段或让其值与 base set 可达性相互矛盾。boot output / runtime info（附录 E）的 `programs[*].status` 字段**必须**取自同一封闭枚举。

---


<!-- Source: 09-state-model.md -->
...
`gen` 与其他 attribute value 一样受 `ATTR_VALUE_MAX_BYTES` 约束；推荐程序之间若需要共享代次词汇表，**应**把 `gen` 控制在 64 bytes 内，并将长描述放入 blob / ref / region metadata，而不是塞入 attribute header。
...
> **strict 顶层 ref materialization** · 当 `features.strict_protocol_mode=true` 且 `features.raw_text_outside_envelope="forbidden"`，并且 `<lmsc_ref>` open 出现在 envelope 栈深 = 0 的 `GENERATING` 位置时，成功 `ref.resolve` 的普通 inline bytes **禁止**仅经 `I-EMIT-ESCAPE` 后直接写入 envelope 外 context。kernel **必须**二选一：(1) 将 resolved content materialize 为一个完整的 kernel-generated `<lmsc_output>` 系统 envelope（或发行版声明的等价 envelope-safe 结构），属性头至少含 `from="system"`、`schema="ref_resolve_result"`，body 仍按 `I-EMIT-ESCAPE` 处理；或 (2) 拒绝本次展开，emit audit `kind=protocol_violation`（`invariant="I-STRICT-ENVELOPE-ONLY"`, `context="ref_expand_strict_top_level"`），并按下方 ref resolve 失败路径向模型呈现系统错误 envelope。该系统 envelope 与 ref 失败错误 envelope 一样由 kernel 直接注入，不触发 self-output-edit observer。发行版若要允许 resolver 返回 typed protocol item 而非普通 bytes，**必须**另行声明 deterministic parser、转义、审计与 conformance；本规范默认的普通 inline bytes 路径不具备该能力。
...
A5 fork 语义仅适用于 `runtime info.kernel.atomics` 包含 A5 的实现。L1 / L2 实现**禁止**暴露可用的 `session.current_handle` / `session.fork` / `session.restore` / `session.drop_fork` 语义；若程序调用这些入口，kernel **必须**返回 `E_ATOMIC_UNSUPPORTED`。本节以下条款定义支持 A5 的实现**必须**满足的完整语义。
...
<!-- Source: 10-scheduling-state-machine.md -->
...
DISPATCHING 中观测到 EOS 的 cleanup 与 §5.3 B2 handler panic / timeout abort 共享同一最小收尾集合：dispatcher frame 栈清理、同一 envelope 的 `on_abort` 至多一次、未完成 stream / generator 取消、`abort` 或 `protocol_violation` audit、以及按触发源选择的主错误码；实现**禁止**只切换到 IDLE 而保留 frame 或未关闭 stream。
...
| `strict_protocol_mode` | `raw_text_outside_envelope` | 行为 |
|---|---|---|
| `false` | `allowed` | RC3 兼容默认行为；无额外 audit 或 violation |
| `false` | `audit-only` | 仅启用裸文本 span audit；不激活 `I-STRICT-ENVELOPE-ONLY` |
| `false` | `forbidden` | **非法配置**；发行版必须在 boot / runtime info 中拒绝该组合，或等价地把 `strict_protocol_mode` 提升为 `true` 后再声明 |
| `true` | `allowed` | strict mode 元数据开启，但 envelope 外普通文本仍允许；不激活 `I-STRICT-ENVELOPE-ONLY` |
| `true` | `audit-only` | envelope 外普通文本允许且记录 audit；不构成 violation |
| `true` | `forbidden` | 激活 `I-STRICT-ENVELOPE-ONLY` 与合法入口集合限制 |

`forbidden` 是唯一会激活 `I-STRICT-ENVELOPE-ONLY` 的取值；`audit-only` 与 `forbidden` 在同一 turn 内**禁止**同时生效。

> `<|lmsc_abort|>` 受 `I-ABORT-SCOPED` 限制不属于 envelope 外合法入口；这意味着 strict + `forbidden` 模式下，模型**无法**在 turn 中段「主动结束当前一切」，只能依赖 EOS 或外部 abort。`<lmsc_ref>` open 在 §4.4 `I-REF-ANY` 下**可**出现在任何 envelope body 与普通 chunk 中；strict + `forbidden` 模式下 envelope 栈深 = 0 处的 `<lmsc_ref>` open **必须**视为合法入口（与 envelope open 同等地位），以避免与 `I-REF-ANY` 冲突——即 entry set 在该模式下为：四种 envelope open、`<|lmsc_suspend|>`、`<lmsc_ref>` open、EOS。但该 ref 成功展开时仍受 §9.3 strict 顶层 ref materialization 约束，resolver 返回的普通 bytes **禁止**直接成为 envelope 外裸文本。
...
> **外部决策器建议**：外部决策器**应**由发行版在 `capabilities.requestable` 配置中声明，并通过 `capability` 程序或等价宿主接口完成 grant/resume；发行版**应**在文档中说明决策器的身份与触发条件，以便程序开发者预期请求的行为。

---


<!-- Source: 12-required-programs.md -->
...
runtime stats # token 使用量、kv 条目、blob 大小、region 数；protocol_overhead；rollback.current_turn_count / rollback.remaining / rollback.storm_locked
...
`runtime stats` 的 rollback 可观测字段为当前 turn 快照：`rollback.current_turn_count` 表示本 turn 已计入 storm-limit 的 rollback 成功 + 失败次数，`rollback.remaining = max(0, limits.rollback_storm_limit - current_turn_count)`，`rollback.storm_locked` 表示 counter 已达上限并锁定至 IDLE。SUSPENDED 不结束 turn，因此这些字段在 suspend/resume 前后**必须**保持同一计数语义。
...
- **后置**：
 ...
 3. emit audit `kind=persona_refresh`（专用 kind，不再与 `cap_grant` 混用），details 至少包含 `diff_blob_id`、`boot_refresh_reason`（发行版自定义）、`persona_region_id`、`active_persona_pin_count` 与 `active_persona_pin_tokens`；若本次调用 supersede 旧 persona-refresh pin，还必须包含 `superseded_persona_region_ids`；默认 `scope=system-only`。
- **pin 生命周期与配额**：启用 `features.persona_refresh=true` 的发行版**必须**在 `runtime info.limits` 声明有限的 `persona_refresh_pin_max_active` 与 `persona_refresh_pin_max_tokens`。kernel 在注入新 persona-refresh pin 前**必须**计算注入后的活跃 persona-refresh pin 数量与 token 总量；若超过任一上限，kernel **必须**二选一：(a) 返回 `E_BOOT_REFRESH_LIMIT` 且不注入新 pin；或 (b) 按发行版声明的确定性策略（例如 newest-N）supersede 足够多旧 persona-refresh pin。superseded pin 的 envelope body 仍保留为历史 context，但不再作为 `attention=pin` 的活跃 pin、不再形成 persona-refresh rollback 硬边界；该 metadata 转换**必须**由同次 `persona_refresh` audit 暴露。发行版**禁止**让 persona-refresh pin 无上限累积。
- **限制**：
 - handler **持有未关闭的流式 `envelope.emit`** 时**禁止**调用（与 §5.2 A5 `fork` 前置同口径；未关闭窗口定义见 §5.3 B3）；满足该条件前提下，作为 `runtime admin` 子命令在 dispatch handler 内调用是合法的（`persona-refresh` 自身即为 `<lmsc_execute>runtime admin persona-refresh ...</lmsc_execute>` dispatch 路径）。违反返 `E_EMIT_STREAM_OPEN`，details 至少含 `reason="streaming_emit_in_progress"`、`context="persona_refresh"` 与 `open_stream_count`。
 - 单 turn 内 ≤ 1 次；超限返 **`E_BOOT_REFRESH_LIMIT`**。本规范统一使用该专用错误码，不复用 `E_PREEMPT_NESTED`。
 - `persona-refresh` **不重置** `ROLLBACK_STORM_LIMIT` 计数器（合规 MUST）。该策略为刻意设计：persona-refresh 是系统角色的特权注入，不代表 turn 逻辑层面的“重新开始”；若允许重置会为恶意或错误程序提供绕过 rollback storm 配额的侧信道。
...
`runtime info.persona` **必须**表示当前有效 persona 快照：boot output 中为 INIT 时的基础 persona；运行时查询中若存在未 supersede 的 persona-refresh pin，则必须反映基础 persona 加活跃 diff 后的有效结果。发行版若因安全策略不能暴露完整文本，**必须**返回脱敏摘要或 `null`，并在 `runtime info.features.persona_refresh` 与 audit 中保留足以关联活跃 persona-refresh pin 的元数据；禁止继续返回过期基础 persona 而不标明脱敏/不可见。
...
<!-- Source: 13-security-invariants.md -->
...
`envelope.emit` 的 `attrs` 值不属于本节四条 body bytes 注入路径；attrs 值若含保留 token 字面或超过 `ATTR_VALUE_MAX_BYTES`，按 §5.3 B3 直接拒绝并返回 `E_EMIT_ESCAPE_VIOLATION`，**禁止**套用上述视觉等价映射或静默截断。
...
- **任何活跃 `attention=pin` region 的边界**——含程序通过 `region.tag(attention=pin,...)` 创建的 pin，以及 `runtime admin persona-refresh` 注入且尚未 supersede 的 `<lmsc_output>` pin output；所有活跃 pin region 均为 rollback 的硬边界。搜索从外部遇到活跃 pin region 起点即停，**禁止**进入 pin region 内部匹配；pin 内部字节不参与外部 rollback pattern 匹配。已按 §12.3.1 生命周期策略 supersede 的 persona-refresh pin 保留为历史 context，但不再作为活跃 pin 或 rollback 硬边界。
...
**Input region 创建规则（规范性）**· kernel **必须**仅在以下路径上为当前 turn 创建 input region 区间，并把该区间纳入上述 rollback 硬边界与 input region 可见性规则：

1. 宿主代表用户向当前 turn 注入用户消息（含 turn 起始首消息与同 turn 内追加用户消息）；
2. C2 `resume` 的外部事件 payload 进入当前 turn 的 token 序列（C2 原子恢复路径，**不**视作程序态 A4 注入）；
3. 发行版在 boot output 或 runtime info 中**显式声明**的「外部输入桥接」路径（如 IDE host、tool host、外部传感器、上游 agent 转写），且该路径在 audit 中以 `source="external_input:<bridge>"` 区分。

以下路径**禁止**创建 input region（不进入 input region 集合）：程序经 `envelope.emit` 写入的任何 envelope 与 body bytes；A4 `token_inject` 全部场景（含 `BeforeNextSample` / `AtEnvelopeClose`）；`runtime admin persona-refresh` 注入的 pin output（其归 persona pin region，与 input region 是不同 region kind）；`ref.resolve` 成功展开的内联替换字节；auto execute scaffold 注入（§10.1.2）；kernel 自身的 boot output（其归 boot region）。任何不在上述允许集合内的注入路径**禁止**被实现以 input region 形式登记；若发行版希望把额外路径纳入 input region，**必须**在 boot output / runtime info 中显式声明并归入第 3 项「外部输入桥接」，否则视为发行版偏离本规范。

同一 turn 内若发生多次允许的输入注入，kernel **必须**为每段输入分别创建 region 并按出现顺序记录为**有序区间集合**；它们各自仍是独立的 rollback 硬边界，**禁止**被合并为单一连续区间或被后续注入跨越。每条 input region 的 audit / runtime query 视图**应**至少含 `kind="input"`、`turn_id`、`range`、`source`（用户消息或 `external_input:<bridge>`）字段。

C2 `resume` 的 `payload=NULL` 或空序列**不**创建零长度 input region；实现必须记录 resume 事件本身，但 `range` 字段应省略或为 `null`，并显式标注 `input_region_created=false`。非空 resume payload 创建的 input region 与普通外部输入同等为 rollback 硬边界；即使 resume 点位于 envelope body 内，后续 rollback 也**禁止**跨入该 payload 区间匹配。

**越界处理**（区分控制流与 audit outcome）· 匹配区间 `[i, i+len(P))` 若有任意字节落在边界外，kernel **必须**：
- **控制流**：KV 不变；若由程序路径 A3 触发则 syscall 返 `E_ROLLBACK_BOUNDARY_HIT`，details `outcome="pattern_out_of_bound"`；若由模型路径触发则不向调用方返错（只留 audit）。
...
- 否则（栈上无任何非 rollback envelope）：`scope = Turn`。

rollback envelope 自身**禁止**参与该推定，即使 `</lmsc_rollback>` close 处理瞬间它仍位于栈顶。该推定结果写入 §14.2 rollback audit details 的 `search_scope` 字段；顶层 `AuditRecord.scope` 继续表示审计可见性。
...
`runtime stats` **必须**暴露当前 turn 的 rollback storm 快照：`rollback.current_turn_count`、`rollback.remaining` 与 `rollback.storm_locked`。这些字段只反映当前 turn 的运行时状态，不进入 boot output schema；SUSPENDED 不重置 counter，故 suspend/resume 前后的 `remaining` 与 `storm_locked` 语义保持连续。
...
> **`source` 字段归因规则（规范性）**· `source` 取值受**封闭枚举**约束：仅允许 `"kernel"` 与 `"program:<name>"` 两种形式；发行版**禁止**引入 `"model"` / `"user"` / `"host"` 等额外取值。kernel 因处理模型生成内容（含 rollback 计算、abort 注入、scaffold 注入、protocol_violation 检测、I-EMIT-ESCAPE 触发、ref resolve 失败 envelope 注入等）而自动 emit 的审计记录，`source` **必须**为 `"kernel"`；与模型行为相关的更细粒度归因（如 rollback 由模型自发或程序触发）**必须**通过 `details` 中的专用字段表达（典型为 `rollback_origin ∈ {"model","program:<name>","kernel"}`、`envelope_from`、`open_source` 等），**禁止**通过修改 `source` 字段携带。`source="program:<name>"` 仅当事件由该程序通过用户态 syscall 直接触发（含 `audit.emit`）时使用。宿主 / runtime 触发的事件归 `source="kernel"` 处理。
...
| `abort` | C3 | envelope_id, reason, **`envelope_from`**, `terminate_reason?`∈{`normal`,`abort`,`timeout`,`cancel`}（流式 emit / observer 路径上存在时必须与 §8.2 terminal frame 口径一致）|
...
| `raw_text_outside_envelope` | `features.raw_text_outside_envelope="audit-only"` 生效时（§10.1.2），envelope 栈深 = 0 处出现的连续裸文本 span（非 violation，仅审计；不要求 `strict_protocol_mode=true`） | `bytes_observed: u64`, `token_count: u32`, `turn_id`, `span_start_token_index`, `span_end_token_index`；**禁止**包含 body 明文。默认 `scope=system-only`，发行版**可**显式降为 `model-visible` |
...
| `envelope_emit` | B3 `envelope.emit` 成功返回 | kind, attrs_keys, body_bytes_len, stream（bool）, chunk_observed?, `terminate_reason="normal"`；异常终止原因由配对的 `abort` / `protocol_violation` audit 或 terminal callback details 承载，发行版若额外 emit terminal `envelope_emit` 记录，也**必须**使用同一 `terminate_reason` 枚举 |
| `envelope_open` | envelope open 事件（scaffold / preempt / ref-resolve 错误 / strict 顶层 materialization 等正文已明定的 kernel 注入路径**必须** emit；模型自发 open **可**省略本 audit，由发行版决定） | `envelope_kind`∈{`execute`,`output`,`edit`,`rollback`}, `envelope_id: string`, `envelope_from`∈{`"model"`,`"program:<name>"`,`"system"`}, `open_source`∈{`"model"`,`"kernel:scaffold"`,`"kernel:preempt"`,`"kernel:ref_resolve_error"`,`"kernel:ref_resolve_strict_top"`}；若顶层 `AuditRecord.envelope_id` 存在，`details.envelope_id` **必须**与之相等；scaffold 注入路径下 `open_source="kernel:scaffold"` 与 `envelope_from="model"` **必须**出现（§10.1.2 / §16.1 C-N4）；preempt 注入路径下 `open_source="kernel:preempt"`，ref-resolve 错误与 strict 顶层 materialization 路径下 `envelope_from="system"` 且 `open_source` 按 §9.3 取值；非 kernel 注入路径下 `open_source` **可**省略或为 `"model"`。默认 `scope=system-only` |
...
`rollback` / `rollback_error` details 中的搜索范围字段**必须**命名为 `search_scope`。顶层 `AuditRecord.scope` 仅表示可见性（如 `system-only` / `model-visible`），**禁止**用作 rollback 搜索范围；旧草案中的 `details.scope` 仅可作为读入兼容 alias，合规实现的新 audit **不得**继续 emit 该字段。
...
<!-- Source: 15-error-code-registry.md -->
...
E_ROLLBACK_PATTERN_NOT_FOUND # 程序路径仅表示 not_found；模型可见兼容层可用 outcome 区分旧式 out_of_bound
...
- **C-1** · 按声明级别实现 §5 原子操作子集。豁免矩阵如下：
 ...
 | **L1** / Tier-B | 24 核心中除 **A5** | A5（`session.fork/restore/drop_fork`，调用返 `E_ATOMIC_UNSUPPORTED`）；A2 / H3 级别相关可选（本级可提供） |
...
- **C-18** · A4 `tokens.inject` **必须**拒绝 `SUSPENDED` 状态下的普通 syscall 调用并返回 `E_INJECT_BAD_POSITION`；C2 `resume` payload 注入仅作为 C2 原子恢复路径存在。任何会导致 envelope 栈超过 `MAX_ENVELOPE_NEST` 的注入**必须**返回 `E_STACK_OVERFLOW`。
- **C-19** · 当 L2 声明 `features.envelope_emit_stream="buffered"` 且启用 `on_envelope_chunk` 时，正常 buffered 路径**必须**在第一次 chunk callback 前预分配最终 `envelope_id` 并复用于最终注入与 audit；只有 open 注入前的 reject 路径可使用 `envelope_id="pending"`。
- **C-20** · 当 `features.persona_refresh=true` 时，`runtime info.limits` **必须**暴露有限的 `persona_refresh_pin_max_active` 与 `persona_refresh_pin_max_tokens`；`runtime admin persona-refresh` **必须**执行该配额策略，并在 audit 中记录活跃 pin 数量、token 总量及被 supersede 的 pin（若有）。
...
- **C-23** · `features.envelope_chunk_observation="self-output-edit"` 只覆盖 emitting handler 自己通过 `envelope.emit(kind=output|edit)` 产生的 output/edit chunk；kernel 直接生成的系统错误 envelope（含 `ref.resolve` 失败错误）**不得**触发该 observer。`ref.resolve` 失败错误与 strict 顶层 ref materialization 系统 envelope **必须**按 §9.3 / §14.2 emit `kind=envelope_open`，其中 `envelope_from="system"`，`open_source` 分别为 `"kernel:ref_resolve_error"` / `"kernel:ref_resolve_strict_top"`，且**禁止**以 `kind=envelope_emit` 表示。
...
- **C-N5** · strict + `raw_text_outside_envelope="forbidden"` 下，envelope 栈深 0 处的 `<lmsc_ref>` 成功展开**不得**把 resolver 返回的普通 inline bytes 直接写入 envelope 外 context。kernel **必须**按 §9.3 materialize 为完整系统 envelope / 等价 envelope-safe 结构，或拒绝展开并 emit `protocol_violation`（`invariant="I-STRICT-ENVELOPE-ONLY"`, `context="ref_expand_strict_top_level"`）。
- **C-N6** · 任何非 rollback envelope 的 open token 与紧随其后的 kernel-owned 属性头**必须**作为不可分割单元写入（§4.2 属性头注入原子性通用规则）：覆盖模型自发 open + 缺省属性头、`envelope.emit`、boot output、persona-refresh、auto execute scaffold、open-tag 形式输入规范化等所有路径；写入期间到达的 C1 / C3 / C4 控制事件**必须**排队等待属性头完成后处理，**禁止**出现「open 已写入但属性头未完成」的中间可观测状态。若放弃写入，**必须**在 open token 写入前取消并**禁止** emit `kind=envelope_open` audit。
- **C-N7** · L0 / L1 下 rollback envelope 触达 `ROLLBACK_PATTERN_MAX_BYTES` 时，kernel **必须**优先强制 `</lmsc_rollback>` close（forced close），仅当强制 close 会同时违反 `I-ROLLBACK-NO-NEST` / `I-ROLLBACK-NO-ATTR` 等不变量时**才**降级为 `<|lmsc_abort|>`（§4.1.2a）。forced close 路径 audit `kind=rollback` details **必须**含 `forced_close=true` 与 `effective_bound="pattern_max_bytes"`，且产生的 rollback envelope body **不得**超过上限；abort fallback 路径**必须**同时 emit `kind=rollback`（含 `forced_close=true`、`forced_close_outcome="abort_fallback"`）与 `kind=protocol_violation`（`invariant=I-ROLLBACK-BOUND`）。**禁止**实现以静默截断或在两条路径间随意切换的方式处理上限。
...
- **C-N17** · C2 `resume` 的空 payload **不得**创建零长度 input region；非空 payload **必须**创建精确覆盖注入 token 区间的 input region，并作为 rollback 硬边界。resume payload 若试图跨越当前 envelope 栈或注入伪造 envelope/control token，**必须**返回 `E_INJECT_BAD_TOKEN` 且不消耗 handle。
- **C-N18** · `runtime admin persona-refresh` **不得**重置 `ROLLBACK_STORM_LIMIT` 计数器；`runtime info.persona` 在 runtime 查询中**必须**表示当前有效 persona（或明确脱敏/不可见），**不得**无标记地返回过期 boot persona。
- **C-N19** · B2 dispatch 拒绝**必须**先归因 base set，再归因 dispatcher frame whitelist；frame whitelist 拒绝使用 `E_CAP_DENIED` + `reason="dispatcher_frame_denied"`，**不得**伪装为程序不存在。`resolve_ref` handler 的 KV 读取只限自身 `program:<resolver-name>/` 动态空间，**不得**读取全局 KV 或其他程序命名空间。
- **C-N20** · dispatch timeout 与 handler panic 并发时只能产生一个主错误码：deadline 已到期则 `E_DISPATCH_TIMEOUT` 优先；panic 已在 deadline 前进入 close 路径则 `E_HANDLER_PANIC`。同 tick 不可区分时 timeout 优先，且 `on_abort` 仍至多一次。
- **C-N21** · `strict_protocol_mode=false` 且 `raw_text_outside_envelope="forbidden"` 是非法配置；`forbidden` 是唯一激活 `I-STRICT-ENVELOPE-ONLY` 的取值。`audit-only` 只产生 raw-text audit，不构成 protocol violation，且不得与 `forbidden` 同 turn 同时生效。
- **C-N22** · L2 rollback body 内的 `<lmsc_ref>` token 只作为 pattern 普通字节处理；实现**不得**在 rollback body 中触发 `ref.resolve`、程序 callback 或程序可见 KV/blob 副作用。

### 16.1.1 最小合规测试矩阵

| 测试项 | L0 / Tier-A | L1 / Tier-B | L2 / Tier-C |
|---|---|---|---|
| `runtime info.kernel.atomics` | 列出 A1-A5、B2-B3、C1-C5、D1-D3、E、F1-F4、G1、H1-H3、I1 | **禁止**列出 A5；A2 / H3 级别相关可选（可列） | **禁止**列出 A5；A2 / H3 可不列 |
...
| `tokens.inject` during `SUSPENDED` | 普通 A4 调用返 `E_INJECT_BAD_POSITION`；C2 payload 仍允许 | 同 L0 | 同 L0 |
| L2 buffered `on_envelope_chunk.envelope_id` | 不适用 | 不适用 | 正常路径预分配且稳定使用最终 id；仅 reject-before-open 路径使用 `pending` |
| `persona_refresh` pin quota（启用时） | 执行并审计有限活跃数与 token 上限 | 同 L0 | 同 L0 |
| rollback audit 搜索范围字段 | details 使用 `search_scope`；不 emit `details.scope` | 同 L0 | 同 L0 |
| attribute value 超限 | `E_EMIT_ESCAPE_VIOLATION`；禁止截断 | 同 L0 | 同 L0 |
| ref.resolve 失败错误 envelope | 不触发 self-output-edit observer | 同 L0 | 同 L0 |
...
- **S-7** · 发行版文档**应**给出 ref 外部 ID canonicalization 约定（如 `base64url(no_padding(utf8_bytes))` 或 `sha256:<hex-prefix>`）以及 collision 策略，并在 resolver manifest / metadata 中登记 `external_id_scheme`。
...
- 把 `details.scope` 用作 rollback 搜索范围，而不是使用 `details.search_scope`；
- 允许 fixed-only capability 出现在 `capabilities.granted` / `capabilities.requestable`，或让 D2 grant fixed-only cap 成功；
- 在 `on_abort` 或 L2 buffered reject-before-open 路径中把 `envelope_id="pending"` 重映射成最终 envelope id；
- 让 `runtime admin persona-refresh` 重置 rollback storm counter；
- 因 dispatcher frame whitelist 拒绝 dispatch 时返回“程序不存在”。

---


<!-- Source: 17-appendices.md -->
...
| `tokens.is_special(id)` | A2 | — | L0 必须提供；L1 可选；L2 可不提供 |
...
| `exec.sandbox(kind, code, stdin?, limits)` | H3 | `exec.sandbox:<kind>` | L0 必须提供；L1 可选；L2 可不提供 |
| `clock.now()` | I1 | — | 可脱敏 |
| `clock.monotonic()` | I1 | — | |
...
机器处理正源为 §7.5 规定的 `lmsc-syscall-schema.yaml`。本人工表用于阅读与索引，**禁止**作为唯一正源；任何方法计数、合并行展开规则、逐方法 `errors[]` 集合与语言绑定生成都**必须**由 schema 校验或生成。错误码的 `origin` 不作为本表逐行字段重复登记，而是由 §15.2 / `lmsc-error-code-registry.yaml` 提供；schema 校验器**必须**把每个 syscall 的 `errors[]` 与错误注册表的 origin 交叉校验。

| Origin 投影 | 适用范围 |
|---|---|
| `model` | 模型直接生成协议 token 触发的事件，如 model-path rollback |
| `program` | 程序显式调用 syscall、handler dispatch 或程序 callback 触发的错误 |
| `kernel` | kernel 内部不变量、配额、状态机或自动清理路径触发的错误 |
| `environment` | 文件、网络、sandbox、时钟等外部环境 backend 触发的错误 |

同一方法可能因不同错误码落入不同 origin；因此 origin 的机器正源是错误码注册表，而不是方法行。
...
features: # 显式 map schema；正文通过 `features.X=value` 形式引用各能力声明
 ...
 persona_refresh: bool # §12.3.1；true 表示暴露 optional runtime admin persona-refresh extension；启用时 limits 中 persona_refresh_pin_* 必须为有限值
 ...
 raw_text_outside_envelope: "allowed" | "audit-only" | "forbidden" # §10.1.2；默认 "allowed"；"audit-only" 可独立生效；"forbidden" 仅在 strict_protocol_mode=true 时合法并启用 §4.4 `I-STRICT-ENVELOPE-ONLY`
...
limits:
 ...
 attr_value_max_bytes: int # 对应 `ATTR_VALUE_MAX_BYTES`（附录 F.1）；所有 kernel-owned attribute header 与会 materialize 为属性头的 attrs 值必须遵守
...
persona: string? # boot output 为 INIT 基础 persona；runtime info 为当前有效 persona，若被策略隐藏则为脱敏摘要或 null
```
...
| `kernel.integration_tier` | `A` | `B` | `C` |
| `kernel.atomics` | 含 A5；A2/H3 必含 | 不含 A5；A2/H3 级别相关可选（可列） | 不含 A5；A2/H3 可不含 |
| `features.invariant_enforcement` | `enforced` | `enforced` | `post-hoc` |
...
| `CHUNK_MAX_TOKENS` | 64 |
| `ATTR_VALUE_MAX_BYTES` | 256 bytes / attribute value（`gen` 建议 ≤ 64 bytes） |
