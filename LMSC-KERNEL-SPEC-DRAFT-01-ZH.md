...
- **前置**：
 ...
 - `fork`：**会话内任一 handler 均**禁止**持有进行中的流式 `envelope.emit`**；违反本条件时**必须**返 `E_EMIT_STREAM_OPEN`，details 至少包含 `reason="streaming_emit_in_progress"`、`context="session.fork"` 与 `open_stream_count`，并应包含可暴露的未关闭 `envelope_id` 列表；持 `session.fork` cap。dispatch 内调用合法——`runtime admin fork`（§12.3）即依赖本路径。（流式 emit 未完结时不可分）
